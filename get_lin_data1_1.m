function [ytt,VD] = get_lin_data1_1

clear all

fi = dir('testes rui/Linear_data');
fi = fi(3:size(fi));
...
fig = input('[y,n]?','s');
clc
disp('Obtaining Linear curves');
for i = 1:size(fi)
    
    [fW,fL,zone] = filename_reader(fi,i,'lin'); % gets the transistors sizes in filename
    ...
    while any(Lorg)% Separting between Ws
        ...
        switch fig
            case 'y'
                ...
                hold off
                saveas(gcf,[cd,'\Figures\LIN\average_L',num2str(Lorg(1,1)*1e6),'_W',num2str(Lorg(1,2)*1e6),date,'.png']);
        end
        if j==1
            yt = y1;
            % the mean, standart deviation and nunber of samples in Id for each value of L,W,VG
