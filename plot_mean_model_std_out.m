function plot_mean_model_std_out(net,L,train)
...
while any(Lorg)% Separting between Ws
    ...
    while any (Worg) % Separating between lines
        ...
        while any(VGSorg)% Separating betwwen points
    ...
    hold off
    saveas(gcf,[cd,'\Figures\MODEL_OUT\average_L',num2str(Lorg(1,1)*1e6),'_W',num2str(Lorg(1,2)*1e6),date,'.png']);
